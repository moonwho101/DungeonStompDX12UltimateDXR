...
void Compute3DSModelNormals(int pmodel_id) {
	...
	for (int frame_num = 0; frame_num < num_frames; frame_num++) {
		...
		// 2. Compute face normals and tangents per object and accumulate into shared vertex indices
		int v_start = 0;
		int face_i_count = 0;
		...
		for (int i = 0; i < num_poly; i++) {
			...
			for (int j = 0; j < num_faces_per_poly; j++) {
				...
				if (g0 >= 0 && g0 < total_num_verts &&
					...
					// Accumulate into shared vertices
		...
		// 3. Normalize accumulated normals and Gram-Schmidt orthogonalize & normalize tangents
		for (int v = 0; v < total_num_verts; v++) {
			float lenN = sqrtf(frame_verts[v].nx * frame_verts[v].nx +
			                   frame_verts[v].ny * frame_verts[v].ny +
			                   frame_verts[v].nz * frame_verts[v].nz);
			if (lenN > 0.00001f) {
				frame_verts[v].nx /= lenN;
				frame_verts[v].ny /= lenN;
				frame_verts[v].nz /= lenN;
			} else {
			...
			float dot = frame_verts[v].nmx * frame_verts[v].nx +
			            frame_verts[v].nmy * frame_verts[v].ny +
			            frame_verts[v].nmz * frame_verts[v].nz;
			frame_verts[v].nmx -= frame_verts[v].nx * dot;
			frame_verts[v].nmy -= frame_verts[v].ny * dot;
			frame_verts[v].nmz -= frame_verts[v].nz * dot;

			float lenT = sqrtf(frame_verts[v].nmx * frame_verts[v].nmx +
			                   frame_verts[v].nmy * frame_verts[v].nmy +
			                   frame_verts[v].nmz * frame_verts[v].nmz);
			if (lenT > 0.00001f) {
				frame_verts[v].nmx /= lenT;
				frame_verts[v].nmy /= lenT;
				frame_verts[v].nmz /= lenT;
			} else {
...
void ComputeMD2ModelNormals(int pmodel_id) {
	...
	for (int frame_num = 0; frame_num < num_frames; frame_num++) {
		...
		// 2. Accumulate face normals and tangents across all triangles
		int i_count = 0;
		for (int i = 0; i < num_poly; i++) {
			...
			for (int j = 0; j + 2 < num_verts_per_poly; j += 3) {
				...
				if (idx0 >= 0 && idx0 < num_verts &&
					...
					// Accumulate face normal and tangent into shared vertex indices
