                    ...
                    if rdx == -wdx and rdz == -wdz:
                        ...
                        if not check_collision(Ox, Oz, cand_name, ang):
                            ...
                            if random.random() < 0.25: # 25% chance per piece
                                ...
                                entities.append({'type': 'LIGHT_SOURCE', 'name': 'Spotlight', 'x': Ox, 'y': s_y, 'z': Oz, 'rot': 0, 'id': entity_id_idx, 'state': 0, 'color': (r_col, g_col, b_col), 'dir': (dx, -1.0, dz)})
    ...
    with open(out_file, 'w') as f:
        ...
        for e in entities:
            ...
            if t == 'wall':
                ...
                direction = e.get('dir', (0.0, -1.0, 0.0))
                f.write(f"LIGHT_SOURCE {e['name']} POS {e['x']:.6f} {e['y']:.6f} {e['z']:.6f}"
                        f" DIR {direction[0]:.6f} {direction[1]:.6f} {direction[2]:.6f} COLOUR {color[0]:.6f} {color[1]:.6f} {color[2]:.6f}\n")
            elif t == 'dframe':
